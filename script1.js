...
function on_click(){  
        ...
        for (let i = 1; i<6; i++){
            ...
            var run = true;
                //logic for green
                    if (correct.some(gren => (gren != null && correct.indexOf(gren) != word.indexOf(gren))))	{
                        run = false;
                    }
                    
                //logic for yellow
                    if (yellow_all.some(yellow => (word.indexOf(yellow) == -1))){
                        run = false;
                }
           
           	//logic for grey
           		if (all_grey.some(grey => (word.indexOf(grey) != -1))){
           			run = false;
           		}
           	return run;
            }
            //loops through guesses to elminate all words which are not compatible
            guesses_cache = words.filter(word => run(word))
		...
		document.getElementById('output').innerHTML = 'guesses are:' + guesses_cache.join(', ');	
	}
	}
